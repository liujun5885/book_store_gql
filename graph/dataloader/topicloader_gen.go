...
func (l *TopicLoader) Load(key string) ([]*model.Topic, error) {
...
func (l *TopicLoader) LoadThunk(key string) func() ([]*model.Topic, error) {
	l.mu.Lock()
...
func (l *TopicLoader) LoadAll(keys []string) ([][]*model.Topic, []error) {
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *TopicLoader) LoadAllThunk(keys []string) func() ([][]*model.Topic, []error) {
	results := make([]func() ([]*model.Topic, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *TopicLoader) Prime(key string, value []*model.Topic) bool {
	l.mu.Lock()
	var found bool
	if _, found = l.cache[key]; !found {
		// make a copy when writing to the cache, its easy to pass a pointer in from a loop var
		// and end up with the whole cache pointing to the same value.
		cpy := make([]*model.Topic, len(value))
		copy(cpy, value)
		l.unsafeSet(key, cpy)
	}
	l.mu.Unlock()
...
func (l *TopicLoader) unsafeSet(key string, value []*model.Topic) {
	if l.cache == nil {
		l.cache = map[string][]*model.Topic{}
	}
	l.cache[key] = value
}
